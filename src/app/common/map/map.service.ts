...
export class MapService {
	...
	private cacheLocation(location: string, position: any) {
		this.locationCache[this.camelize(location)] = position;
	}

	private isLocationCached(locationKey): boolean {
	...
	public getGeoLocation(location: string): Observable<any> {
		...
		if (this.isLocationCached(locationKey)) {
			return Observable.of(this.locationCache[locationKey]);
		} else {
